...
> **Executed 16 June 2026, with two raters.** The check was run early (on the pre-run pool, per Implementation Log 9.8) with two of the three planned raters — Prof. Dash was unavailable (his blank workbook is retained; the analysis script reruns with three raters if his ratings are added later). Headline result: **composite-level agreement is strong (Spearman ρ = 0.856 vs the averaged human composite; gate agreement 77–87%), while criterion-level pooled κ = 0.29–0.45 falls below the pre-locked 0.60 and triggered the required investigation** — which traced the gap to two identified mechanisms (the documented gated-task Contextual-Independence leakage, which never reaches the index, and a genuine human-vs-model calibration gap on Capability Match). Full results, mechanisms, and implications in Implementation Log 9.9 and Decision History 5.28; the limitation is recorded in Section 6.11.
...
> **Executed 16 June 2026 (Implementation Log 9.13).** Tier 3 (Eloundou) ρ = 0.858 → **PASS** (≥ 0.55). Tier 6 (Webb) discriminant gap = 0.688 → **PASS** (≥ 0.15), with strongly negative correlations against the pre-LLM automation measures. Tier 5 (SML) ρ = −0.162, behaving as a discriminant rather than the expected moderate convergent — reported as a finding. Tiers 1 (multi-LLM reliability) and 2 (human validity) are addressed separately (Tier 2 done early, Sections 5.28 / 9.9; Tier 1 pending an Anthropic key).
...
| 16 Jun 2026 | Pre-run operational changes for incremental funding and early validity (no re-pilot needed) | (a) `03_score_tasks.py` now halts cleanly on out-of-credit (`insufficient_quota`) instead of churning, so the full run can be funded in increments and resumed; (b) the full-run input is now `master_tasks_shuffled.csv`, a fixed-seed permutation of `master_tasks.csv`, so the pre-run validation pool is the run's first chunk (zero re-scoring); (c) the Tier-2 human validity check is brought forward — score the shuffled prefix with the remaining ≈ $3.80, draw a random 100, rate before the full run; (d) the rater guide's criteria are now **verbatim-identical** to the system prompt (28/28 phrases verified) to remove any wording confound, and a verbatim exact-prompt PDF was produced for the supervisors. None of these touch the locked rubric/schema/model/temperature/aggregation. Details in Implementation Log 9.8. |
| 16 Jun 2026 | Tier-2 validity sample changed from stratified to **simple random** | On the supervisors' recommendation, the 100-task human-validity sample is now a simple random draw (fixed seed) rather than 25 per composite-score band. Rationale: a population-representative κ (agreement on a *typical* task), and no analyst degrees of freedom in sample construction. Trade-off (sparse-region coverage; the prevalent zero-gated category depressing Cohen's κ — the prevalence/kappa effect) is handled by also reporting percent agreement, the confusion matrix, per-criterion κ, and the sample's score distribution — not by re-stratifying. Threshold κ ≥ 0.60 retained. Supersedes 5.19; documented in Decision History 5.27 and Section 3.4. |
| 16 Jun 2026 | Pre-run pool scored (734 tasks, $3.79); clean credit-halt validated in production; §3.10 physicality check brought forward and **passed** (0/24 IS=1 hybrids leak) | The shuffled-master run scored 734 tasks with zero failures before halting cleanly at credit exhaustion. First corpus distribution: gate rate 55%, eight composite values, mean 0.329. The audit's one untested risk — physical-presence double-counting on non-gated IS=1 hybrids — did not materialise. PASS-subset OV↔CtxI correlation (0.72) logged as an Output-D watch item; IS=0⟹CM=0 restated as near-universal (95%). Implementation Log 9.9. |
| 16 Jun 2026 | **Tier-2 human validity executed (two raters): composite corroborated (ρ = 0.856), criterion-level κ below threshold — investigated and explained** | Run early, with the researcher and Dr. Mukherjee (Prof. Dash unavailable; deviation documented, rerunnable with his ratings). Composite: ρ 0.856, gate agreement 77–87% (κ up to 0.743), MAE 0.174. Criterion-level pooled κ 0.29–0.45 → the pre-locked "<0.40 investigate" band → investigation attributes it to (a) gated-task Contextual-Independence leakage that never reaches the index (humans corroborate the §6.5 diagnosis), and (b) a genuine Capability-Match calibration gap (humans more generous than the model on digital tasks; conservative direction for the index). Prompt stays locked; production run proceeds; Output C restricted to PASS subset with a CM caveat. Decision History 5.28; Implementation Log 9.9; limitation §6.11. |
...
### 5.27 Why the Tier 2 Validity Sample Was Changed from Stratified to Simple Random (16 June 2026)
...
### 5.28 Tier-2 Executed Early with Two Raters: Results, the Investigation, and What They Mean (16 June 2026)

**The deviation.** The protocol specified three raters (researcher + both supervisors). Prof. Dash could not allocate the rating time, so the check ran with two: the researcher and Dr. Mukherjee (who additionally annotated every one of her 100 rows). This is a documented deviation, not a redesign: his blank workbook is retained, the analysis (`src/06_analyze_validation.py`) reruns unchanged if his ratings are added, and the two-rater results are reported as such. Consequences of n=2: (a) Krippendorff's α has wide uncertainty and cannot distinguish "one idiosyncratic rater" from "genuinely contested construct"; (b) the protocol's "averaged human rating" is ill-defined when the two raters disagree by one (the average x.5 must be rounded — we round half-up and report per-rater κ as the primary numbers, the rounded-average as sensitivity); (c) one of the two raters is the researcher, so the independent-rater signal rests more heavily on Dr. Mukherjee — her numbers are reported separately throughout, and they are consistently the *stronger* of the two, which limits the self-rating concern.

**Results (n = 100; no task flagged ambiguous; full tables in Implementation Log 9.9).** Composite level: Spearman ρ = 0.856 between the LLM composite and the averaged human composite (per-rater 0.720 / 0.850); mean absolute error 0.174; gate agreement 77% on the averaged-human gate (per-rater 79% / 87%, κ 0.591 / 0.743). Criterion level: Information Sufficiency κ = 0.550 / 0.670 (binary gate decision κ 0.536 / 0.743); Objective Verifiability κ = 0.304 / 0.479; Contextual Independence κ = 0.091 / 0.274; Capability Match κ = 0.191 / 0.287; pooled across criteria 0.308 / 0.447 per rater, 0.287 against the rounded average. Inter-rater: pooled κ = 0.521, ordinal Krippendorff α = 0.633 (IS 0.666, OV 0.401, CtxI 0.402, CM 0.684).

**Verdict against the pre-locked bands.** The composite — the quantity the index actually publishes — is strongly corroborated by human judgement (ρ 0.86 with no threshold gymnastics). The criterion-level pooled κ falls in the "< 0.40 triggers investigation" band, so the investigation was run, and it attributes the gap to two specific mechanisms rather than diffuse noise:

1. **The gated-task Contextual-Independence leakage (pre-documented in Section 6.5) accounts for the bulk.** On LLM-gated tasks, Contextual-Independence agreement is 16–27%; on PASS tasks it rises to 52–73% (κ up to 0.51; 0.418 on the both-digital subset). The model writes "requires physical presence" into Criterion 3 on physical tasks; the human raters score the criterion as written — Dr. Mukherjee's notes repeatedly make the separation explicit (e.g. *"physical involvement required even though outcome is objective and requires little contextual understanding"* → her CtxI = 2 where the model leaked CtxI = 0). The humans thereby *corroborate the audit's leakage diagnosis*. Because every such task is gated by Information Sufficiency, **these disagreements never touch the published index** — which is exactly why the composite agreement is strong while criterion-level κ is weak.
2. **A genuine calibration gap on Capability Match where it matters.** On the both-digital subset, Capability-Match agreement collapses to ~27% (κ ≈ 0.02–0.05) for *both* raters, in a consistent direction: the humans read fluent digital tasks as "at or above average human" (2) where the model scores "useful but below" (1). Notably this is the *opposite* of the RLHF self-overconfidence direction that Section 3.9 flagged as the a-priori risk — the evaluating model is *more conservative about model capability than the human raters*. If the humans are right, the index *understates* exposure on capable digital tasks; the composite effect is bounded (±0.125 per step, and the strong composite ρ shows ordering survives). It is reported as the principal criterion-level validity limitation (Section 6.11), not silently absorbed.

A third, background factor bounds everything: inter-rater α of 0.633 means the two humans themselves agree only moderately — particularly on Objective Verifiability and Contextual Independence (α ≈ 0.40) — so no evaluator, human or model, could score far above that ceiling against "the human rating."

**Implications adopted.** (a) The composite index proceeds to the full run unchanged — the prompt stays locked; nothing in these results identifies a fixable prompt defect that the gate does not already absorb. (b) The binding-constraint decomposition (Output C) and any per-criterion reporting are computed on the **PASS subset only** and carry the Capability-Match caveat — this was already the §6.6 reporting decision; the validation now attaches numbers to it. (c) Criterion-level κ is reported in the paper exactly as measured, with this mechanism analysis alongside.
...
### 6.11 Criterion-Level Human Agreement Is Materially Weaker Than Composite-Level Agreement

The Tier-2 validity check (executed 16 June 2026 with two raters; Implementation Log 9.9, Decision History 5.28) found strong human corroboration of the **composite** — Spearman ρ = 0.856 against the averaged human composite, gate agreement 77–87% — but criterion-level pooled κ of 0.29–0.45, below the pre-locked 0.60. The investigation attributed this to: (a) the gated-task Contextual-Independence leakage (Section 6.5), which never enters the index but dominates criterion-level disagreement counts; (b) a genuine calibration gap on **Capability Match** among digitally-accessible tasks (κ ≈ 0.02–0.05; the human raters read fluent digital tasks as "at or above average human" where the model scores "useful but below" — the model is *more conservative about model capability than human raters*, the opposite direction of the §3.9 self-overconfidence concern); and (c) a moderate human-human ceiling (ordinal α = 0.633) that bounds any evaluator's achievable κ.

**Implications.** The published index (the composite) is human-corroborated. Per-criterion artefacts — the binding-constraint decomposition (Output C) and criterion-level interpretation — are reported on the PASS subset only and carry an explicit caveat that Capability Match's 1-vs-2 distinction has weak human agreement; conclusions are not built on that distinction alone. If the human raters' more generous capability reading is correct, the index *understates* exposure on capable digital tasks by up to 0.125 per affected task — a conservative direction for an exposure index. Two-rater caveats (n = 2, researcher among them) are in 5.28; the check is re-runnable with the third rater's workbook at any time.

**Why criterion validity is weak — root cause (not uniform; two distinct sources).** Comparing human–human agreement (Krippendorff α) with human–model agreement (κ) per criterion is diagnostic: Information Sufficiency α 0.67 / κ ≈ 0.6 — a *reliable construct the model tracks well*; Objective Verifiability α 0.40 / κ ≈ 0.39 and Contextual Independence α 0.40 / κ ≈ 0.18 — the humans agree only *moderately with each other*, because "is success objectively checkable?" and "is this relationship-dependent?" are genuinely contestable judgements, so **no evaluator could score high** and the low κ partly reflects an inherently fuzzy construct rather than a measurement defect; Capability Match α 0.68 / κ ≈ 0.24 — the anomaly, where humans agree strongly *with each other* but systematically disagree *with the model*, which is consistently **more conservative about AI capability** than the (AI-researcher) raters. The Capability-Match gap is therefore a genuine **calibration disagreement about a contested empirical question** (what current models can do), not noise. The composite nonetheless validates (ρ 0.85 human; 0.86 Eloundou) because averaging four imperfect-but-roughly-unbiased criteria cancels idiosyncratic noise, and the dominant determinant of the composite — the gate — is carried in practice by Information Sufficiency, the one reliable criterion. **The holes thus live in the criteria that pose genuinely hard or contested questions; they are largely intrinsic to the task, not fixable by prompt edits** — which is why the right response is honest caveating, not further iteration.

### 6.12 The Immeasurability of the Relational/Tacit Barrier Is a Finding, Not a Defect

The Contextual-Independence criterion is, on three independent measures, the least-measurable of the four: lowest human–human agreement (Krippendorff α 0.40), lowest human–model agreement (Cohen κ ≈ 0.18), and lowest text-decodability (BERT R² 0.46 vs ~0.60 for the others). A naïve reading treats this as a weakness of the criterion's wording. We argue the opposite, and report it as a substantive finding.

**The construct it measures is the one the automation literature identifies as intrinsically hardest to formalise.** Contextual Independence operationalises the *social-intelligence / tacit-knowledge bottleneck* — Polanyi's Paradox ("we know more than we can tell"), formalised for automation by Autor (2015); the "social intelligence" engineering bottleneck of Frey & Osborne (2013); and the rising, automation-resistant social-skill premium documented by Deming (2017). Whether a task "genuinely requires a real-time human relationship or insider tacit context" is **a question without ground truth**: it depends on unstated context, professional norms, and the irreducible judgement of what cannot be written down. It is therefore *expected* — not anomalous — that reasonable human raters disagree with one another, that a model disagrees with all of them, and that the signal is poorly recoverable from text. **A criterion measuring an immeasurable construct should look immeasurable.** If Contextual Independence had scored as cleanly as the physical/digital criterion, that would have been evidence it was measuring something easier (and less interesting) than the tacit barrier it is meant to capture.

**This is why we decompose rather than collapse.** Single-label exposure measures (Eloundou's E-tiers, Felten's AIOE) fold this dimension into one number, where its low measurability is invisible — silently degrading the whole score without anyone seeing where the noise enters. Our four-criterion design *isolates* the relational barrier, so its immeasurability becomes legible and can be reported as a result: **the frontier of non-automatability is, by its nature, the frontier of non-measurability.** The cost of doing so honestly is a low criterion-level κ on that dimension; the benefit is that the well-measured dimensions (physical, verifiable) carry the validated composite while the hard dimension is named rather than buried. This reframes Section 6.6 and the criterion-validity limitation (6.11): the weakness is located, explained, and converted into a contribution.

### 6.13 Navigating the Limitations for Publication (Literature-Backed Positioning)

The project's limitations are real but are, in almost every case, *constraints the entire task-based exposure literature shares and has not solved*. The defensible posture is to state each plainly, show it is field-wide, and lean the headline claims on the validated composite. A reviewer-facing map:

| Limitation | Why it is not disqualifying | Literature / evidence |
|---|---|---|
| **Single-model dependence** (index is GPT-4o's judgement) | Universal to LLM-as-judge exposure work; we *report* reliability bounds (Tier 1, pending) rather than ignore them — more than the field standard. | NBER WP 35110 (2025); Eloundou et al. 2023 also single-evaluator. |
| **Weak criterion-level validity** (OV/CtxI κ low) | The constructs are inherently contested; the *composite* is what is validated (ρ 0.85 human, 0.86 Eloundou, the wage inversion). Report the decomposition as "constraint pressure," not certified attribution. | Sections 6.11–6.12; Autor 2015; Frey & Osborne 2013; Deming 2017. |
| **Capability Match is a self-assessment** | The feared RLHF over-confidence did *not* appear — the model was, if anything, *conservative* vs humans (a calibration gap, bounded ±0.125/task, conservative for an exposure index). | Section 5.28 / 6.11; cf. the self-assessment concern in 3.9. |
| **Substitution-only scope** (augmentation scores 0) | A deliberate construct choice for cleaner validity; augmentation is a *separate* measure. Our with-tools framing aligns with Eloundou's E2 (ρ 0.87). | Section 3.3 / 6.2; Anthropic 2025 (augmentation half-weighting). |
| **Gate compresses the bottom half** (55% at 0) | Inherent to any physical/digital cut; we *recover* sub-structure via the latent cuts (frontier tasks; embodiment-only-protected work, §9.15). | Section 9.15. |
| **Task bundling / additive aggregation** | No published exposure index corrects for task interdependence. | Section 6.1; Autor 2013. |
| **Composite is BERT-distillable (ρ 0.96)** | Distillation (trained on LLM labels), not redundancy; a *scalability* contribution, not evidence the LLM was unneeded. | Section 9.14. |
| **No external ground truth for "true" GenAI exposure** | Addressed by *discriminant* validation — we correlate negatively with the methodologically-independent pre-LLM measures (Webb patents, Frey-Osborne surveys), the strongest available anchor. | Sections 6.9, 9.13. |

The single genuinely open item is **single-model dependence**, which the Tier-1 Claude reliability run (pending an Anthropic key) directly addresses. Everything else is either closed, converted into a finding, or a documented scope choice shared by the field.
...
The methodology is fully specified and **Part 1 is essentially complete** — the index is built, validated (human, convergent, discriminant), characterised for reproducibility, and analysed. The table tracks each step. Only the **Tier-1 Claude reliability check** (needs an Anthropic key) and the **supervisor synthesis write-up** remain open.
...
| Full GPT-4o scoring run | Submit all 18,796 task statements via the OpenAI **synchronous** Chat Completions API (Section 5.22) at T = 0, fixed seed. Log every prompt and response. Resumable. | **Complete (16 June 2026)** — Prof. Dash ran all 18,796 on the college key; 0 failures; published index at `data/processed/scored_master_tasks_shuffled.csv`. See Implementation Log 9.11. |
| Human-LLM validity sample (Tier 2) | Simple random sample of 100 tasks (fixed seed; Section 5.27) rated independently by the researcher and supervisors; κ vs. averaged human + inter-human α. | **Complete (16 June 2026)** — composite ρ 0.85, gate agreement 77–87%; criterion caveats explained. Two raters (Dash absent). Sections 5.28, 9.9, 6.11 |
| Aggregation & analytical outputs (A–D) | Occupation-level IM-weighted exposure, dispersion, barrier decomposition, dimensionality. | **Complete (16 June 2026)** — `data/processed/occupation_exposure_index.csv` (923 occs); §9.12 |
| Robustness check (no analyst tasks) | Recompute the index without analyst-coded tasks; compare rankings. | **Complete (16 June 2026)** — Spearman ρ 0.9995 with the full index; imputation inconsequential; §9.12 |
| External validation (Tiers 3/5/6) | Spearman ρ vs. Eloundou (convergent), Webb (discriminant), SML; pre-locked thresholds ρ_Eloundou ≥ 0.55 and gap ≥ 0.15. | **Complete (16 June 2026)** — Eloundou ρ 0.858 PASS; discriminant gap 0.688 PASS; negative vs. old-automation; §9.13 |
| BERT reproducibility analysis | Embed task texts (`all-mpnet-base-v2`); predict LLM composite; R², residuals. | **Complete (16 June 2026)** — R² 0.62 task / ρ 0.96 occ (distillation); §9.14. (`all-MiniLM` second model optional, not run.) |
| Distribution & case-study review | Score distribution, top/bottom occupations, sectoral and structural inferences. | **Complete (16 June 2026)** — sectors, wage inversion (+0.48 vs robots −0.49), embodiment moat (92%), latent cuts; §9.15 |
| Post-scoring audit | Review zero-gated tasks for interpretation artefacts (Section 3.10). | **Partial (16 June 2026)** — the physicality double-count check passed clean (§9.11); a broader zero-gate artefact review remains optional (expected small per 3.10). |
| Claude reliability sample (Tier 1) | Score ~1,800 tasks with Claude on the identical prompt; inter-model κ ≥ 0.70 — addresses single-model dependence. | **Pending** — needs an Anthropic API key. The one open external item (Sections 6.4, 6.13). |
| Expert spot-check | Domain-expert review of 25–30 occupations. | **Dropped (11 May 2026)** — supplementary tier removed; Tier 2 carries the human comparison. See Section 5.18. |
| Part 1 deliverable | Validated index, occupation summary, reliability + reproducibility reports, for supervisor review before Part 2. | **In progress** — index, validation, and reproducibility are complete; the synthesis write-up for supervisors remains. |
...
### 9.8 Pre-Run Operational Changes for Incremental Funding and Early Validity (16 June 2026)
...
### 9.9 Pre-Run Pool Scored, Tier-2 Validity Executed, and the Corpus Pool Audit (16 June 2026)

**Pool run (734 tasks, $3.79).** The shuffled master file was scored until the $5 credit ran out: 734 tasks in ~74 minutes, **zero terminal failures, zero content retries**, and the new credit-exhaustion handler worked exactly as designed in production — a clean halt at task 735 with partial results written and a resume instruction, no retry churn. The run resumes from the checkpoint when the account is topped up; the 734 scores are final (they are the opening segment of the production run, per 9.8).

**First real corpus distribution (n = 734).** Gate rate **55%** (404 gated / 330 PASS). The composite spans **eight values** — 0, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.0 — mean 0.329, non-zero mean 0.732. Per-criterion value use: Information Sufficiency {0: 401, 1: 24, 2: 309}; Objective Verifiability {0: 182, 1: 133, 2: 419}; Contextual Independence {0: 423, 1: 132, 2: 179}; Capability Match {0: 384, 1: 288, 2: 62}. Composite 1.0 (fully substitutable) is 8.4% of tasks; Capability Match = 2 is 18.8% of PASS tasks — the sparse ceiling predicted from the pilot.

**The Section 3.10 physicality double-count check, brought forward — PASSED.** The corpus produced the first genuine Information-Sufficiency = 1 hybrids (24 tasks, 3.3%; the pilot had zero, leaving this the audit's one untested pathway). For all 24, the Contextual-Independence and Objective-Verifiability reasonings were scanned for physical-presence language: **0 of 24 leak.** The hybrids are scored on their merits (advising students, monitoring telemetry, proctoring exams), land at composites 0.25–0.625, and the feared double-counting of physicality on non-gated tasks did not materialise. The residual-risk flag in Section 6.5 is hereby resolved for the pool; the check reruns on the full corpus after the production run.

**PASS-subset criterion correlations (Spearman, n = 330).** Information Sufficiency is now decoupled from everything (0.12–0.29). The notable pair is **Objective Verifiability ↔ Contextual Independence at 0.72** (Contextual Independence ↔ Capability Match 0.50): among digital tasks, subjective-success tasks tend also to be context-dependent (advisory, judgement, stakeholder work). The criteria remain conceptually distinct and do diverge on a material minority of tasks, but Output D's factor analysis should expect a shared "judgement/relational" factor; reported as a finding, not a defect.

**The IS = 0 ⟹ CM = 0 property is near-universal, not absolute (correction to 5.26/6.6 phrasing).** In the pilot all 38 physical tasks double-gated; in the pool **381 of 401** IS = 0 tasks have CM = 0 (95%). The 20 exceptions score CM = 1 with "can assist with the informational part" reasoning (e.g. authenticating historical documents, customer fee estimates). All 20 are gated by Information Sufficiency regardless, so the index is unaffected; the property is restated as "near-universal" wherever cited. Conversely, 3 tasks gate on Capability Match alone — all worker-self-development tasks ("participate in training/continuing education"), where the model reasons an AI cannot *be* the trainee; a sensible, documented edge case.

**Other integrity checks.** Eight task texts appear under more than one occupation: 6/8 score identically across occupations; 2/8 differ by one composite band — consistent with the measured ~2.7% single-step noise floor plus legitimate occupation-context differences (the prompt includes the occupation title by design). Occupation-level means on the pool (small n per occupation, per §6.7 caveat) are face-valid: machine operators and clinical/physical care at 0, Fundraisers (0.81) and Financial Risk Specialists (0.63) at the top. A regex scan found **zero** PASS tasks whose modulating-criterion reasoning invokes physical presence (two regex hits were negations — "without requiring physical presence").

**Tier-2 validity executed (two raters).** The blind workbooks came back from the researcher and Dr. Mukherjee (100/100 rows each, no flags; Dr. Mukherjee annotated every row). Analysis script: `src/06_analyze_validation.py`. Results in full:

| Measure | vs Mehta | vs Mukherjee | vs averaged human |
|---|---|---|---|
| Composite Spearman ρ | 0.720 | 0.850 | **0.856** |
| Gate agreement (κ) | 79% (0.591) | 87% (0.743) | 77% (0.558) |
| Composite MAE | 0.230 | 0.136 | 0.174 |
| Information Sufficiency κ (lin-wtd) | 0.550 (0.609) | 0.670 (0.731) | 0.551 |
| Objective Verifiability κ | 0.304 | 0.479 | 0.223 |
| Contextual Independence κ | 0.091 | 0.274 | 0.067 |
| Capability Match κ | 0.191 | 0.287 | 0.172 |
| Pooled criterion κ (400 ratings) | 0.308 | 0.447 | 0.287 |

Inter-rater (human–human): pooled κ 0.521, ordinal Krippendorff α 0.633 (IS 0.666, OV 0.401, CtxI 0.402, CM 0.684). Subset diagnostics that drive the interpretation: on LLM-gated tasks, Contextual-Independence agreement is 16–27% (the §6.5 leakage — the model floors Criterion 3 on physical tasks; the humans score the relational dimension as written); on the both-digital subset it recovers to κ 0.42 (Mukherjee). Capability Match on the both-digital subset is the genuine weak point: ~27% agreement, κ ≈ 0.02–0.05 for both raters, humans consistently *higher* than the model. Full mechanism analysis and adopted implications in Decision History 5.28; limitation recorded as Section 6.11. The composite index is human-corroborated; the prompt remains locked; the production run proceeds.

### 9.10 Production Run Handed to Prof. Dash; Validation to Be Re-Confirmed Against Final Scores (16 June 2026)

The college provided Prof. Dash a higher-tier OpenAI API key (access to `gpt-4o-2024-11-20` plus newer models; far higher limits than the personal Tier-1 key). The production run is therefore handed to him to execute. **Decision: run all 18,796 tasks fresh on the college key as a single consistent run**, rather than resuming from the 734-task pre-run pool. Rationale: a single account/session producing the entire published index is cleaner and avoids any mixed-provenance question; the ~$3.79 saving from resuming is immaterial on the college account. The 734 pool scores are retained for reference but are not part of the published index.

**Implication for Tier-2 validity.** The validation reported in 9.9 / 5.28 was computed against the *pool* scores. When the production run completes, the Tier-2 κ/α analysis is **re-computed against the final published scores** for the same 100 task_ids, reusing the existing human rating sheets unchanged (they are keyed by task_id). Expected to be near-identical to the pool result (same model, prompt, temperature 0, seed; only the ~2.7% determinism floor can move a handful of scores), but re-run for exactness. `src/06_analyze_validation.py` requires only that the `KEY` file's scores be refreshed from the final `scored_master_tasks_shuffled.csv`.

**Handoff package** (`handoff/GenAI_Exposure_FinalRun.zip`): the two locked source files (`rubric_prompt.py`, `03_score_tasks.py`), the run input (`master_tasks_shuffled.csv`), a self-guiding Jupyter notebook (`RUN_THE_SCORING.ipynb`), `requirements.txt`, and a guide (`00_READ_ME_FIRST.md` + `.pdf`) written for both Prof. Dash and an assisting AI agent. The guide's load-bearing invariant is that the **model, prompt, schema, temperature, seed, and aggregation must not be changed** — the package ships them locked and the script uses them automatically; the run resumes on interruption via its own checkpoint. Batch API was not used: the synchronous script is already validated, simpler for a non-technical operator, works on any tier, and on the higher-tier key completes in ~10–12 h.

### 9.11 Production Run Complete; Final Corpus, Audit Closure, and Validation Re-Confirmed (16 June 2026)

Prof. Dash executed the full run on the college key. It paused once at 81% (after task 15,267) — a clean external interruption (the progress file ended on a normal task with zero errors, most consistent with an overnight kernel/sleep interruption, not a failure) — and was resumed with the same command, completing all 18,796 tasks.

**Run quality.** 18,796 / 18,796 scored; every master `task_id` covered exactly (0 missing, 0 extra, 0 duplicate); **0 terminal failures, 0 content retries**; every PASS/GATED row carries all nine fields. The professor's complete progress file is now the canonical output, and `data/processed/scored_master_tasks_shuffled.csv` (the published per-task index) was rebuilt from it.

**Final corpus distribution (18,796).** Gate rate **54.8%** (10,297 gated / 8,499 PASS); composite mean 0.327, non-zero mean 0.722; full eight-value gradient — 0 (54.8%), 0.25 (0.9%), 0.375 (5.2%), 0.5 (4.2%), 0.625 (9.8%), 0.75 (6.3%), 0.875 (10.4%), 1.0 (8.4%). Per-criterion: IS {0:10204, 1:605, 2:7987}; OV {0:4439, 1:3457, 2:10900}; CtxI {0:10829, 1:3446, 2:4521}; CM {0:9870, 1:7344, 2:1582}. This matches the 734-task pre-run pool almost exactly (54.5% gate, mean 0.329), confirming the pool was representative.

**Cross-account reproducibility.** For the 100 validation `task_id`s, the personal-key pool run and the college-key full run differ on **15 of 400 criterion scores (3.8%)** — consistent with the documented ~2.7% temperature-0 floor — with **0 of 100 gate decisions changing**. Determinism holds across accounts, models held constant.

**§3.10 / §6.5 physicality watch-item — CLOSED on the full corpus.** 605 IS=1 hybrids (3.2%). A crude regex flags 66 PASS tasks with physical-presence words in a modulating criterion's reasoning, but reading them shows these are overwhelmingly **false positives** — "manuals" (documents), "presence of records/students" (existence, not a person), and "physical principles/properties/phenomena" (the task's subject matter) — all correctly scored. The few arguable cases (~3: Medical Equipment Repairers, Model Makers, Petroleum Engineers) are defensible on-site/fabrication judgements. Reassigning every one of the 66 to its digitally-accessible value shifts a composite by at most 0.25 (mean 0.045), affects 0.35% of tasks, and only ever in the conservative (exposure-understating) direction. The feared physicality double-counting did not materialise at scale. (IS = 0 ⟹ CM = 0 holds for 9,777 / 10,204 = 95.8%, as predicted; the exceptions are gated regardless.)
...
### 9.12 Occupation-Level Index (Outputs A–C) and Criterion Dimensionality (Output D) (16 June 2026)
...
### 9.13 External Validation — Convergent (Eloundou) and Discriminant (Webb) (16 June 2026)

The occupation-level index was correlated against published comparators, all obtained from Eloundou et al.'s public replication repository (`openai/GPTs-are-GPTs`): their occupation exposure (`occ_level.csv`) and a multi-measure file (`autoScores.csv`) carrying Webb 2020, Brynjolfsson–Mitchell–Rock SML, Felten AIOE, and Frey–Osborne. Alignment: Eloundou matched **all 923** occupations by O*NET-SOC; the others matched 750 at the 6-digit SOC level. Spearman, unweighted across occupations, per the pre-registered design (`src/09_external_validation.py` → `data/processed/external_validation_results.csv`).

**Both pre-locked criteria PASS, decisively:**
- **Tier 3 convergent — Eloundou.** ρ(ours, Eloundou E1 human-labelled) = **0.858** (model-labelled 0.911; E2-with-tools 0.873). Threshold ≥ 0.55 → **PASS**. The two measures share method (O*NET task text + an LLM rubric), so this is *consistency*, not independent verification — but it is strong and reassuring.
- **Tier 6 discriminant — Webb.** On the common 750-occupation set, ρ(Eloundou) = 0.864 vs ρ(Webb-AI patent) = **0.176**; discriminant **gap = 0.688** (threshold ≥ 0.15 → **PASS**). The full pattern is textbook: the index correlates **negatively** with the pre-LLM automation measures — Webb-robots −0.637, Webb-software −0.171, Frey–Osborne −0.235 — and near-zero with broad-AI measures (Felten AIOE 0.214). It agrees with the other generative-AI measure and *disagrees* with old-automation measures, exactly as pre-registered. This is the single result that most strengthens the index: it demonstrates the index captures **generative-AI-specific substitution, not generic computerisability** — and because the discriminant comparators use entirely different methods (patent text, expert surveys), this is genuine external evidence, not shared-method consistency.

**Tier 5 (SML) deviated from the pre-locked expectation, interpretably.** Expectation was "moderate positive"; result is ρ = **−0.162**. SML measures suitability for *supervised* ML (prediction/perception), which includes physical/perceptual tasks the LLM-substitution index gates out; here it behaves as a discriminant, not a convergent. Reported as a finding (Tier 5 carried no hard threshold).

**Disagreements are design-explained, not errors.** Where we score *higher* than Eloundou (Bookkeeping Clerks 0.98 vs E1 0.31; Medical Transcriptionists 0.80 vs 0.23): our disambiguation rule assumes modern digital tools, aligning us with Eloundou's *E2-with-tools* measure (ρ 0.87) more than their tool-free E1. Where Eloundou scores *higher* than us (Psychiatrists, Music Therapists, Nannies, Counter Clerks): our Design-Y gating and the Contextual-Independence criterion explicitly protect relational/care work that their task-share average exposes — a defensible, arguably superior, design difference. The external check therefore both validates the index and illuminates exactly where our construct choices bite.

### 9.14 BERT Reproducibility (Section 3.5) — the LLM index is ~62% text-decodable per task, ~96% distillable at occupation level (16 June 2026)

All 18,796 task texts were embedded with sentence-BERT (`all-mpnet-base-v2`, 768-d, GPU) and the LLM scores predicted from the embeddings alone (`src/10_bert_reproducibility.py`; embeddings cached and gitignored).

**Task-level: the "mixed" band — the methodology's expected, most informative outcome.** Predicting the LLM composite from embeddings gives R² = **0.62** (5-fold CV 0.620 ± 0.010; Ridge and gradient-boosting agree, so no non-linearity is being missed), RMSE 0.24. The PASS-subset gradient is equally decodable (R² 0.61) — the nuanced modulation among non-gated tasks is *not* more reasoning-driven than the gate. The gate (gated vs not) is highly text-predictable (AUC 0.946). Per criterion, **Contextual Independence is the least text-decodable (R² 0.46)** vs IS/OV/CM ~0.60–0.64 — the relational/tacit criterion is where the LLM most goes beyond the wording, and (consistently) also the noisiest in human validation. Reading: ~60% of the LLM's per-task scoring is recoverable from surface semantics; ~40% reflects context-sensitive judgement embeddings cannot capture — squarely the "mixed" band (Section 3.5).

**What the 40% residual actually is — concrete.** Out-of-fold residual analysis (|residual| > 0.3 on 19.6% of tasks) shows the LLM's value-add is overwhelmingly **resolving physical/digital ambiguity that the wording disguises**, in both directions: it scores *up* tasks whose words sound physical but whose content is digital ("monitor gauges/meters," "mark frames" → composite 1.0 where text predicts 0), and scores *down* tasks whose words sound clerical but whose action is embodied ("scan records into electronic format," "gather materials to be filed" → 0 where text predicts ~1.0). This is genuine contextual reasoning embeddings miss — though a minority of the up-residuals are *debatable* over-digitisation (e.g., "remind patients to take medications" → 1.0), so the residual is reasoning + defensible disambiguation + some over-aggressive disambiguation + stochastic noise, not purely "value-add."

**Occupation-level: highly distillable.** At the occupation level the BERT-predicted index correlates **ρ = 0.958** with the true index — the task-level residuals largely average out over an occupation's ~20 tasks. *Important framing:* this is **distillation, not independent reproduction** — the regression is trained on the LLM's own labels, so it cannot exist without first running the LLM. The contribution is therefore "the index, once built, can be cheaply distilled into an embedding model and extended to new tasks/O*NET versions/languages for free," not "the LLM was unnecessary." Honestly stated, this is a real scalability result *and* a mild caution that at the aggregate level the index is close to a sophisticated function of task-text content, with the LLM's irreducible value concentrated in correctly classifying the ~20% ambiguous-wording tasks.

### 9.15 Deeper Inferences: Sectors, the Wage Inversion, the Embodiment Moat, and the Immeasurable Relational Barrier (16 June 2026)

`src/11_deep_analysis.py` mines the decomposition for concrete, reportable findings.

**Sectoral pattern (SOC major groups).** Most exposed: Computer & Math (0.70), Office & Admin Support (0.66), Business & Financial (0.58), Architecture & Engineering (0.56), Legal (0.48), Management (0.47). Least: Construction & Extraction (0.08), Food Prep (0.13), Farming (0.13), Installation/Repair (0.13), Production (0.14). The ordering is the generative-AI signature — cognitive/clerical/analytical at the top (notably **Computer & Math is the single most-exposed group**, which prior automation spared), physical/manual/care at the bottom — matching Eloundou and Felten. Office & Admin and Healthcare Practitioners show the widest within-group spread (SD ≈ 0.2): internally split groups (data-entry vs. executive assistants; medical-records specialists vs. surgeons).

**The wage inversion — the distinctive GenAI signature.** Exposure correlates **+0.48 with occupation mean wage**, rising monotonically across wage quintiles (Q1 0.17 → Q5 0.45), whereas **Webb's robot exposure correlates −0.49 with wage**. GenAI *inverts the historical skill-bias of automation* — it reaches high-wage cognitive work that robots and computerisation left alone. ~21% of US employment is in high-exposure (≥ 0.6) occupations; employment-weighted mean exposure 0.34. This is the single most policy-salient result and is convergent with Eloundou's headline.

**Embodiment is the dominant moat; the capability frontier is thin.** A per-occupation barrier decomposition assigns **845 of 923 occupations (92%) a dominant protective barrier of physical embodiment**, 51 relational, 15 verifiability, 1 capability, 11 little-protected. The "capability frontier" — digital tasks the model judges itself *unable* to do (IS = 2 & CM = 0) — is only **39 tasks (0.2%)**. Reading: the model believes it can *attempt* almost all digital work; what protects occupations is overwhelmingly the physical body, not cognitive difficulty — so the moat *weakens as robotics advances* (central to Part 2). We additionally flag **427 "embodiment-only-protected" tasks (IS = 0 but CM ≥ 1)** — cognitively tractable work shielded only by an in-person/physical convention (e.g., "teach sociology," "consult with clients to prepare plans") — the first to fall if delivery moves remote or the physical step is offloaded.

**The relational barrier is intrinsically immeasurable — a finding, not a defect (Section 6.12).** Three independent signals converge on Contextual Independence as the least-measurable criterion: lowest human–human agreement (α 0.40), lowest human–model agreement (κ ≈ 0.18), and lowest text-decodability (BERT R² 0.46). This is the Polanyi's-Paradox / social-intelligence bottleneck (Autor 2015; Frey & Osborne 2013; Deming 2017) made visible: the dimension hardest to measure is the one capturing the most human-distinctive, least-automatable capability. That our four-criterion design *isolates* this dimension — rather than collapsing it into a single label as Eloundou/Felten do — is precisely what lets us see it, and is the methodological justification for decomposing rather than collapsing (Section 6.12).
