...
def main():
    print("Loading MAEI Data...")
    maei_df = pd.read_csv(MAEI_PATH)

    print("Loading BLS OEWS 2023 Data... (This takes a moment due to Excel size)")
    ...
    print("Merging datasets on SOC code...")
    ...
    print(f"Total MAEI occupations evaluated: {len(maei_df)}")
    print(f"Successfully matched with BLS Employment Data: {merged_df['TOT_EMP'].notna().sum()}")
    print(f"Successfully matched with BLS Annual Wage Data: {merged_df['A_MEDIAN'].notna().sum()}")
    
    print(f"Total employed workers evaluated: {merged_df['TOT_EMP'].sum():,.0f}")
