...
def main():
    ...
    print(f"Correlation between MAEI and Median Wage: {corr_wage:.3f}")
    ...
    print(wage_q_summary.to_string(index=False))
    ...
    for idx, row in high_paid_exposed.iterrows():
        print(f"* {row['Occupation']} - Wage: ${row['A_MEDIAN']:,.0f} | MAEI: {row['MAEI_2026_Score']} | Workers: {row['TOT_EMP']:,.0f}")
    ...
    for idx, row in mass_employment.iterrows():
        print(f"* {row['Occupation']} - Wage: ${row['A_MEDIAN']:,.0f} | MAEI: {row['MAEI_2026_Score']} ({row['Risk_Level']}) | Workers: {row['TOT_EMP']:,.0f}")
